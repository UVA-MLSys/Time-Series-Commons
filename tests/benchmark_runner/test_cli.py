...
def test_real_run_preserves_adapter_unsupported_reason(tmp_path, monkeypatch):
    data_path = tmp_path / "toy.csv"
    pd.DataFrame(
        {
            "timestamp": pd.date_range("2024-01-01", periods=4, freq="h"),
    ...
    class Adapter:
        def predict(self, context_df, future_df, task):
            raise cli.UnsupportedTaskError(
                "Moirai2",
    ...
    assert exit_code == 0
    assert result["status"] == "skipped"
    assert result["error"]["reason"] == "resource_budget_exceeded"
