...
def test_build_dry_run_summary_counts_tasks_by_fields():
    tasks = [
        SimpleNamespace(model_id="Model-A", suite_dataset_id="forecast_dataset", io_mode="UV-UV", horizon_id="short"),
        SimpleNamespace(model_id="Model-A", suite_dataset_id="forecast_dataset", io_mode="MV-UV", horizon_id="short"),
        SimpleNamespace(model_id="Model-B", suite_dataset_id="forecast_dataset", io_mode="UV-UV", horizon_id="medium"),
    ]
...
def test_dry_run_filters_by_repeatable_options_and_prints_json(tmp_path, capsys):
    ...
    assert summary["models"] == {"Model-A": 1}
    assert summary["datasets"] == {"forecast_dataset": 1}
...
def test_real_run_writes_skipped_result_when_model_dependency_is_missing(tmp_path, monkeypatch):
    ...
    def missing_adapter(model_id, device_map="auto"):
        raise cli.MissingDependencyError("Model-A", ("forecast-package",), "https://example.test")
